...
const h=document.getElementById("start");console.log("Found start button:",h),h?(h.addEventListener("click",()=>{console.log("Start/Next button clicked"),console.log("Current room state:",{categories:a.categoryManager.categories,query:a.query,mode:a.mode}),a.query.subjects=a.categoryManager.categories,console.log("Starting with categories:",a.categoryManager.categories),a.message(USER_ID,{type:"start"})}),console.log("Start button handler attached")):console.error("Start button not found!"),document.getElementById("toggle-correct").addEventListener("click",()=>{a.message(USER_ID,{type:"toggle-correct"})});const i=document.getElementById("toggle-ai-mode");i&&i.addEventListener("change",b=>{a.message(USER_ID,{type:"toggle-ai-mode",aiMode:b.target.checked})});const j=document.getElementById("toggle-rebuzz");j&&j.addEventListener("change",b=>{a.message(USER_ID,{type:"toggle-rebuzz",rebuzz:b.target.checked})});const k=document.getElementById("toggle-show-history");k&&k.addEventListener("change",b=>{a.message(USER_ID,{type:"toggle-show-history",showHistory:b.target.checked})});const l=document.getElementById("toggle-timer");l&&l.addEventListener("change",b=>{a.message(USER_ID,{type:"toggle-timer",timer:b.target.checked})});const m=document.getElementById("type-to-answer");m&&m.addEventListener("change",b=>{a.message(USER_ID,{type:"toggle-type-to-answer",typeToAnswer:b.target.checked})});const n=document.getElementById("set-strictness");n&&n.addEventListener("input",b=>{const c=Number.parseInt(b.target.value,10);if(Number.isNaN(c))return;const d=Math.max(0,Math.min(100,c));b.target.value!==d+""&&(b.target.value=d);const e=document.getElementById("strictness-display"),f=document.getElementById("strictness-note");e&&(e.textContent=d),f&&(f.textContent=describeStrictness(d)),a.message(USER_ID,{type:"set-strictness",strictness:d})});const o=document.getElementById("reading-speed"),p=document.getElementById("reading-speed-display");o&&o.addEventListener("input",b=>{const c=Number.parseInt(b.target.value,10);if(!Number.isNaN(c)){const b=Math.min(100,Math.max(0,c));p&&(p.textContent=b),o.value!==b+""&&(o.value=b),console.log("[Science Bowl] Reading speed slider adjusted:",{rawValue:c,value:b}),a.message(USER_ID,{type:"set-reading-speed",readingSpeed:b})}}),document.getElementById("pause").addEventListener("click",()=>{console.log("Pause button clicked"),a.message(USER_ID,{type:"pause"})}),document.addEventListener("keydown",b=>{if("INPUT"!==b.target.tagName)switch(b.key){case" ":return a.message(USER_ID,{type:"buzz"});case"n":return hideAIHelpSection(),a.message(USER_ID,{type:"start"});case"p":return a.message(USER_ID,{type:"pause"});case"s":return hideAIHelpSection(),a.message(USER_ID,{type:"start"})}});// Also clear AI panels immediately when clicking the Start/Next button
