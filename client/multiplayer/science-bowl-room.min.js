...
const d=c?4:10;console.log("Points to add:",d);const e=b+d;console.log("New score:",e),a.textContent=`SCORE: ${e}`};function updateTimerDisplay(a){const b=Math.floor(a/10);document.getElementById("timer").querySelector(".face").textContent=b,document.getElementById("timer").querySelector(".fraction").textContent="."+a%10}// Utility to update the player list sidebar
function updatePlayerList(a){const b=document.getElementById("player-list-group"),c=document.getElementById("player-list-group-hr");if(b){const d=Object.values(a||{});return 0===d.length?(b.classList.add("d-none"),void(c&&c.classList.add("d-none"))):void(b.innerHTML="",d.forEach(a=>{const c=document.createElement("li");c.className="list-group-item d-flex justify-content-between align-items-center",c.textContent=a.username||a.userId||"Player";const d=document.createElement("span");d.className="badge bg-primary rounded-pill",d.textContent=null==a.score?0:a.score,c.appendChild(d),b.appendChild(c)}),b.classList.remove("d-none"),c&&c.classList.remove("d-none"))}}// Listen for multiplayer events to update player list
function handlePlayerEvents(a){("connection-acknowledged"===a.type||"join"===a.type||"give-answer"===a.type)&&(a.players?updatePlayerList(a.players):window.room&&window.room.players&&updatePlayerList(window.room.players)),"players-update"===a.type&&(console.log("[Client] Received players-update:",a.players),updatePlayerList(a.players))}// Patch the main message handler to call handlePlayerEvents
const originalOnMessage=onmessage;function onmessagePatched(a){const b="string"==typeof a?JSON.parse(a):a;return handlePlayerEvents(b),originalOnMessage(a)}// Initialize the room
window.onmessage=onmessagePatched,"undefined"!=typeof socket&&(socket.send=onmessagePatched),document.addEventListener("DOMContentLoaded",()=>{console.log("DOM loaded, setting up event handlers"),console.log("Using global room instance:",window.room);const a=window.room;console.log("Category manager:",a.categoryManager),console.log("Initial categories:",a.categoryManager.categories),document.getElementById("start").disabled=!1,document.getElementById("start").textContent="Start/Next",document.getElementById("buzz").disabled=!0,document.getElementById("pause").disabled=!1,document.getElementById("pause").textContent="Pause";// Load saved settings
